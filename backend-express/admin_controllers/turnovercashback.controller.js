                ...
                .on('end', function (data) {
                    ...
                    if (accountData.length > 0) {
                        for (key in accountData) {
                            if (key === '0') {
                                if (accountData[key][3] !== 'Client Account Number' && accountData[key][10] !== 'Trx Amt Prog Cur' && accountData[key][13] !== "Affiliate's Fee") {
                                    return res.send({ success: false, message: 'Invalid file format.' });
                                }
                            }
                            if (key !== '0') {
                                const trackingCode = accountData[key][2];
                                const regDate = accountData[key][7];
                                const firstName = accountData[key][4];
                                const LastName = accountData[key][6];
                                const ecoAccount = accountData[key][3];
                                const purchaseVolume = accountData[key][10];
                                const revenue = accountData[key][13];
                                ...
                                newReport.account.commission = Math.round(revenue * 100) / 100;
                                newReport.account.cashback = Math.round((revenue * 0.785) * 100) / 100;;
                                newReport.account.cashbackRate = newReport.account.commission === 0 ? '0%' : ((newReport.account.cashback / purchaseVolume) * 100).toFixed(2) + '%';
                                        ...
                                        Report.findOneAndUpdate(
                                            { 'account.accountId': accountId, monthId: monthId },
                                            {
                                                $set: {
                                                    ...
                                                    'account.commission': Math.round(revenue * 100) / 100,
                                                    'account.cashback': Math.round((revenue * 0.785) * 100) / 100,
                                                    'account.cashbackRate': ((((Math.round((revenue * 0.785) * 100) / 100) / purchaseVolume) * 100).toFixed(2)) + '%'
                                                }
                                            },
                                        ).then((report) => {
                                            return Promise.all([Account.findOne({ 'account.accountId': report.account.accountId }), report])
